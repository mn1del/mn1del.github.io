...
function makeXMotor(inputs,pinion){
    //split body into 3 just for colouring
    var body0 = CSG.cube({corner1: [0,0,0],corner2:[inputs.properties.stepW,inputs.properties.stepW,inputs.properties.stepL/5]}).setColor([150/255,150/255,150/255]);
    var body1 = CSG.cube({corner1: [0,0,inputs.properties.stepL/5],corner2:[inputs.properties.stepW,inputs.properties.stepW,inputs.properties.stepL*0.8]}).setColor([0,0,0]);
    var body2 = CSG.cube({corner1: [0,0,inputs.properties.stepL*0.8],corner2:[inputs.properties.stepW,inputs.properties.stepW,inputs.properties.stepL]}).setColor([150/255,150/255,150/255]);
    var body = body0.union([body1,body2]).center("x","y");

    var shaft = CSG.cylinder({start:[0,0,0],end:[0,0,inputs.properties.stepSL],radius:inputs.properties.stepSD/2,resolution:res}).setColor([211/255,211/255,211/255]);


    //create cutouts for the 4 corners, and the screw hole
    var cutoutShift = (inputs.properties.stepW - inputs.properties.stepHS)/2;
    var cylCutOut0 = CSG.cylinder({start:[0,0,0],end:[0,0,inputs.properties.stepL/5],radius:inputs.properties.stepHTh,resolution:res}).setColor([150/255,150/255,150/255]);
    var cylCutOut1 = CSG.cylinder({start:[0,0,inputs.properties.stepL/5],end:[0,0,inputs.properties.stepL*0.8],radius:inputs.properties.stepHTh,resolution:res}).setColor([0,0,0]);
    var cylCutOut2 = CSG.cylinder({start:[0,0,inputs.properties.stepL*0.8],end:[0,0,inputs.properties.stepL-2],radius:inputs.properties.stepHTh,resolution:res}).setColor([150/255,150/255,150/255]);
    var cylCutOut = cylCutOut0.union([cylCutOut1,cylCutOut2]);
    var sqCutOut0 = CSG.cube({corner1:[0,0,0],corner2:[inputs.properties.stepHTh*2,-inputs.properties.stepHTh,inputs.properties.stepL/5]}).center("x").setColor([150/255,150/255,150/255]);
    var sqCutOut1 = CSG.cube({corner1:[0,0,inputs.properties.stepL/5],corner2:[inputs.properties.stepHTh*2,-inputs.properties.stepHTh,inputs.properties.stepL*0.8]}).center("x").setColor([0,0,0]);
    var sqCutOut2 = CSG.cube({corner1:[0,0,inputs.properties.stepL*0.8],corner2:[inputs.properties.stepHTh*2,-inputs.properties.stepHTh,inputs.properties.stepL-2]}).center("x").setColor([150/255,150/255,150/255]);
    var sqCutOut = sqCutOut0.union([sqCutOut1,sqCutOut2]);
    sqCutOut = sqCutOut.union(sqCutOut.rotateZ(-90));

    var screwHole = CSG.cylinder({start:[0,0,0],end:[0,0,inputs.properties.stepL],radius:inputs.properties.stepHTh/2,resolution:res}).setColor([150/255,150/255,150/255]);
    var cutout = cylCutOut.union([screwHole,sqCutOut]).translate([-inputs.properties.stepL/2 + cutoutShift,-inputs.properties.stepL/2 + cutoutShift,0]);

    //apply the cutouts to the body - four corners
    body = body.subtract([
        cutout,
        cutout.rotateZ(90),
        cutout.rotateZ(180),
    ...
    //locate the body below the z axis
    body = body.translate([0,0,-inputs.properties.stepL]);

    var motor = body.union(shaft);

    //set connector(s)
    motor.properties.cPinion_Motor = new CSG.Connector([0,0,0],[0,0,1],[1,0,0]);

    motor = motor.connectTo(motor.properties.cPinion_Motor,pinion.properties.cPinion_Motor,false,0);

    return motor;
}

function makeSpindleMount(inputs,zPlungeArm){//zPlate){
    //make constituent parts
    var spBase = CSG.cube({corner1: [0,0,0],corner2:[inputs.properties.spinMtBasL,inputs.properties.spinMtH,inputs.properties.spinMtBasT]}).setColor([211/255,211/255,211/255]);
    var spBody = CSG.cube({corner1: [0,0,0],corner2:[inputs.properties.spinMtBodL,inputs.properties.spinMtH,inputs.properties.spinMtBasBodD - inputs.properties.spinMtBasT]})
        .translate([(inputs.properties.spinMtBasL - inputs.properties.spinMtBodL)/2,0,inputs.properties.spinMtBasT]).setColor([211/255,211/255,211/255]);
    var spHole  = CSG.cylinder({start: [0,0,0], end: [0, inputs.properties.spinMtH, 0], radius: inputs.properties.spinBD/2, resolution: res}).setColor([211/255,211/255,211/255])
        .translate([inputs.properties.spinMtBasL/2,0,inputs.properties.spinMtD - inputs.properties.spinBD/2 - inputs.properties.spinMtT]);
    var spCircle = CSG.cylinder({start: [0,0,0], end: [0, inputs.properties.spinMtH, 0], radius: inputs.properties.spinBD/2 + inputs.properties.spinMtT, resolution: res}).setColor([211/255,211/255,211/255])
        .translate([inputs.properties.spinMtBasL/2,0,inputs.properties.spinMtD - inputs.properties.spinBD/2 - inputs.properties.spinMtT]);

    //bring together
    var spinMt = spBase.union([spBody,spCircle]).subtract(spHole);

    //define connector to zPlate, and to spindle
    spinMt.properties.cZPA_SpM = new CSG.Connector([inputs.properties.spinMtBasL/2,0,0],[0,0,1],[0,1,0]);
    spinMt.properties.cSpindle = new CSG.Connector([inputs.properties.spinMtBasL/2,0,inputs.properties.spinMtD - inputs.properties.spinMtT],[0,0,-1],[0,1,0]); //connects on the front inside edge of the spondle hole

    //position on zPlate.
    spinMt = spinMt.connectTo(spinMt.properties.cZPA_SpM,zPlungeArm.properties.cZPA_SpM,false,0);
